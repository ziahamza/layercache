...
The task performs deterministic PBKDF2 computation and writes a small output file. Calibration uses Node's `process.cpuUsage()` to measure user and system CPU consumed by a short probe, then fixes the iteration count for all samples. Time waiting for a processor on a busy host cannot reduce that count. The default target is two CPU seconds per cold task; `--target-duration` accepts 2s through 30s. Every cold sample must still record at least one second of task execution. No sleeps or invented producer durations supply the measured saving.

Durations cover the complete `layercache run` invocation, including Turbo startup and final summary reconciliation. Clone creation, daemon setup, calibration, and subsequent report queries are outside that interval. JSON preserves every duration, the output digest, native Turbo summaries, CLI output, and Layer Cache reports, plus the installed binary digest, client versions, platform, and calibration count. The calibration record includes measured user/system CPU microseconds, observed probe wall time, and the requested CPU target. Probe wall time is diagnostic evidence and does not size the workload. The median calculation uses all successful samples without trimming outliers. It retains ratios above one and fails when a restore is slower than the gate allows.
...
The reusable GitHub workflow `.github/workflows/performance.yml` runs this gate for CI and release publication and uploads its JSON evidence. Statistics and calibration arithmetic tests use fixed, hand-calculated examples. A short Node probe checks that real CPU measurements are available without asserting machine speed. Full cold/warm trials run only through the explicit performance command.
