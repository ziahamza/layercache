...
const fixture = spawn(process.execPath, [resolve('qa/cloud/fixture.ts')], {env: process.env, stdio:['ignore','pipe','inherit']});
...
try {
  const origin = await new Promise<string>((resolveReady,reject) => {
    ...
    lines.on('line',line=>{
      ...
      try { const info: {ready?:boolean;root?:string;origin?:string}=JSON.parse(line); if(info.ready&&info.origin){clearTimeout(timeout);lines.close();console.log(`Cloud fixture: ${info.root}`);resolveReady(info.origin);} } catch(error){clearTimeout(timeout);reject(error);}
    });
  });
  browser=spawn(process.execPath,[resolve('qa/cloud/browser.ts')],{env:{...process.env,CLOUD_ORIGIN:origin},stdio:'inherit'});
