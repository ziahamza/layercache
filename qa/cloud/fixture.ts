...
const github=createServer(async(req,res)=>{
  ...
  if(url.pathname==='/authorize'){
    ...
    if(req.method!=='GET'||url.searchParams.get('client_id')!=='fixture-client'||redirect!==expectedCallback||url.searchParams.get('scope')!=='read:user repo'||url.searchParams.get('code_challenge_method')!=='S256'||!state||!challenge){res.writeHead(400);res.end('{}');return;}
  ...
  if(url.pathname==='/token'){
    ...
    if(req.method!=='POST'||!identity||form.get('client_id')!=='fixture-client'||form.get('client_secret')!=='fixture-oauth-secret'||form.get('redirect_uri')!==identity.redirect||createHash('sha256').update(form.get('code_verifier')||'').digest('base64url')!==identity.challenge){res.writeHead(403);res.end('{}');return;}
...
expectedCallback=origin+'/auth/callback';
