...
function simulatorCompatibility(target) {
  ...
  const digest = (0, import_node_crypto4.createHash)("sha256").update(JSON.stringify([target.os, target.arch, target.xcode, target.sdk])).digest("hex");
...
var provider = {
  ...
  async resolveBuildCache(props, options) {
    ...
    try {
      ...
      let name;
      const result = await client(resolved).restoreManaged(identityForBuild(props, resolved), async (destination2) => {
        const names = await (0, import_promises7.readdir)(destination2);
        name = names[0];
        if (names.length !== 1 || !name || !name.endsWith(props.platform === "ios" ? ".app" : ".apk")) throw new Error("Unexpected native artifact");
        if (props.platform === "ios" && process.platform === "darwin") await validateSimulatorApp((0, import_node_path13.join)(destination2, name), await detectSimulatorTarget(props.projectRoot));
      });
      ...
      return (0, import_node_path13.join)(destination, name);
    } catch {
      warn();
      return null;
    }
